'''
VEM segmentation algorithm altered to apply to 2D (as opposed to 3D) markov fields
'''
...
class Segmentation(object):
    ...
    def __init__(self, data, local_values=0, rws_range=(-32, 32),
                 mask=None, mu=[[0, 0], [0, -31.5]],
                 sigma=[[[.5, 0], [0, 2]], [[300, 0], [0, 3]]],
        ...
        X, Y = np.where(mask)
        XYZ = np.zeros((X.shape[0], 2), dtype='intp')
        XYZ[:, 0], XYZ[:, 1] = X, Y
        self.XYZ = XYZ
    ...
    def normalized_external_field(self):
        # the probabilities without considering the neighbor
        # relationships!
